...
class FileWriter {
    ...
    async writeProjectFiles(projectResponse) {
        try {
            ...
            if (!projectResponse || !projectResponse.files || projectResponse.files.length === 0) {
                throw new Error('项目响应数据无效或没有文件需要写入');
            }

            // 获取项目根目录句柄
            const rootDirHandle = await this.getProjectRootDirectory(projectResponse.rootPath);
            
            // 写入所有文件
            const writePromises = projectResponse.files.map(file => 
    ...
    downloadProjectFiles(projectResponse) {
        try {
            console.log('使用下载方式保存项目文件');
            
            projectResponse.files.forEach((file, index) => {
